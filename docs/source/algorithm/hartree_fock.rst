...
原子中 :math:`N` 个电子的非相对论性 Hamiltonian：
...
其中 :math:`\psi_i` 为自旋轨道（spin-orbital），包含空间和自旋部分。
...
在正交归一化约束 :math:`\langle \psi_i | \psi_j \rangle = \delta_{ij}` 下，导出 **Hartree-Fock 方程**。
...
- :math:`v_H`: Hartree 势（经典电子排斥）
- :math:`\hat{K}`: 交换算符（量子效应）
...
:math:`\varepsilon_i` 为轨道能，:math:`\psi_i` 为自旋轨道。
...
- :math:`v_H(r)`: 径向 Hartree 势
- :math:`v_x(r)`: 交换势（需特殊处理）
...
:math:`f_{n\ell}` 为占据数（考虑自旋和磁量子数简并）。
...
**特点**：
- 自旋 :math:`\alpha` 和 :math:`\beta` 电子共享同一空间轨道
- 占据数 :math:`f_{n\ell} = 2(2\ell + 1)`（满壳层）
- 交换算符包含所有占据轨道
...
**特点**：
- 自旋 :math:`\alpha` 和 :math:`\beta` 有独立空间轨道
- 占据数按自旋分离：:math:`f_{n\ell\sigma}`
- 交换仅在同自旋间
...
**缺点**：
- 自旋污染（:math:`\langle \hat{S}^2 \rangle \neq S(S+1)`）
- 破坏自旋对称性
...
其中 :math:`C` 为 Clebsch-Gordan 系数。

**选择规则**：
- :math:`|\ell - \ell'| \leq k \leq \ell + \ell'`
- :math:`k + \ell + \ell'` 为偶数
...
其中：
- :math:`\varepsilon_i`: 轨道能
- :math:`E_H`: Hartree 能（双计数校正）
- :math:`E_x`: 交换能（双计数校正）
