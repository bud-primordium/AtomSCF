...
其中 :math:`u_{n\ell}(r) = r R_{n\ell}(r)` 为径向波函数，:math:`v_{\mathrm{eff}}(r)` 为有效势能。
...
4. **更新密度**: 用新轨道重建电子密度
5. **收敛检查**: 判断密度变化是否低于阈值
6. **密度混合**: :math:`\rho^{(n+1)} = \alpha \rho_{\mathrm{new}} + (1-\alpha) \rho^{(n)}`
...
.. list-table::
   ...
   * - 方法
     - 有效势 :math:`v_{\mathrm{eff}}`
     - 优点
     - 缺点
   * - RHF
     - :math:`v_H + \hat{K}_{\mathrm{RHF}}`
     - 精确交换，闭壳层准确
     - 开壳层强制配对，无关联
   * - UHF
     - :math:`v_H^{\sigma} + \hat{K}_{\sigma}`
     - 自旋极化，开壳层改进
     - 自旋污染，无关联
   * - LSDA
     - :math:`v_H + v_{xc}[n_{\uparrow}, n_{\downarrow}]`
     - 包含关联，速度快
     - 局域近似，低估带隙
...
- 原子单位: :math:`\hbar = m_e = e = 4\pi\epsilon_0 = 1`
- 能量单位: Hartree (Ha), 1 Ha = 27.211 eV
- 长度单位: Bohr (a₀), 1 a₀ = 0.529 Å
- 角动量量子数: :math:`\ell = 0` (s), 1 (p), 2 (d), ...
- 自旋标记: :math:`\sigma = \uparrow` (up), :math:`\downarrow` (down)
